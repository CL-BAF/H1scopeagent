...
from h1scopeagent.db.models import BrowserScoutEntry
...
async def scout_with_safety(
    ...
    try:
        with get_db() as db:
            entry = BrowserScoutEntry(
                program_handle=program_handle,
                original_url=sc["original_url"],
                final_url=sc["final_url"],
                in_scope=sc.get("in_scope", True),
                manual_review_required=sc.get("manual_review_required", False),
